...
export default function Home() {
  ...
  const previousHR =
    history.length > 1 ? history[history.length - 2].heartRate || 0 : 0;
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center min-h-screen bg-slate-50">
        ...
        <p className="text-slate-500">
          Wait for a smartwatch to sync to the Realtime Database.
        </p>
      </div>
    );
  }
      ...
      <div className="hidden md:flex items-center justify-between pb-6 mb-6 border-b border-slate-100">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 text-slate-500">
            Real-time health monitoring overview
          </p>
        </div>
        <div className="flex items-center gap-4">
          <DeviceSelector className="scale-110" />
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden relative">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <Badge
              variant="outline"
              className={`${hrStatusColor} flex items-center gap-1.5 rounded-full px-2.5 py-0.5 animate-pulse`}
            >
              <span
                className={`h-1.5 w-1.5 rounded-full ${hrIndicatorColor}`}
              ></span>
              LIVE
            </Badge>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex items-center gap-1.5 mt-2 text-xs text-slate-500">
              <div
                className={`h-2 w-2 rounded-full ${hrDiff > 0 ? "bg-red-500" : "bg-emerald-500"}`}
              ></div>
              <span>
                <span
                  className={`font-medium ${hrDiff > 0 ? "text-red-600" : "text-emerald-600"}`}
                >
                  {hrDiff > 0 ? "+" : ""}
                  {hrDiff} bpm
                </span>{" "}
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden relative">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-1.5 mt-2 text-xs text-slate-500">
              <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-none font-semibold rounded text-[10px] uppercase">
                Normal
              </Badge>
            </div>
          </CardContent>
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden relative">
          ...
          <CardContent>
            <div className="flex items-baseline gap-2">
              <span
                className={`text-3xl font-bold tracking-tight ${healthText === "Normal" ? "text-emerald-600" : "text-yellow-600"}`}
              >
                {healthText}
              </span>
            </div>
        ...
        <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden relative">
          <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <Badge
              variant="outline"
              className={`${device.status === "Online" ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-slate-50 text-slate-600 border-slate-200"} flex items-center gap-1.5 rounded-full px-2 py-0.5`}
            >
              {device.status}
            </Badge>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span
                  className="text-lg font-bold text-slate-800 tracking-tight line-clamp-1"
          ...
          <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white overflow-hidden">
            <CardHeader className="flex flex-row items-center justify-between pb-4 border-b border-slate-100">
              <div>
                ...
                <CardDescription>
                  Real-time analytics over the last points.
                </CardDescription>
              </div>
              <div className="flex flex-col items-end gap-1 text-right">
                <Badge
                  variant="outline"
                  className={`${hrStatusColor} px-2.5 rounded-full font-semibold mb-1`}
                >
                  <span
                    className={`h-1.5 w-1.5 rounded-full ${hrIndicatorColor} mr-1.5 animate-pulse inline-block`}
                  ></span>
                  LIVE
                </Badge>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card className="rounded-2xl border-slate-200/60 shadow-sm bg-white">
              <CardHeader className="pb-2">
                ...
                <CardDescription>
                  Steps progress over recent syncs
                </CardDescription>
              </CardHeader>
            ...
            <CardContent>
              <ul className="space-y-2 mt-1">
                <li className="flex gap-2">
                  <div
                    className={`mt-1 h-1.5 w-1.5 rounded-full ${healthText === "Normal" ? "bg-blue-500" : "bg-red-500"} shrink-0`}
                  />
                  <p className="text-sm text-slate-700 leading-snug">
                    {healthText === "Normal"
                      ? "Heart rate is completely stable."
                      : "Heart Rate anomaly detected!"}
                  </p>
                </li>
                <li className="flex gap-2">
                  <div
                    className={`mt-1 h-1.5 w-1.5 rounded-full ${device.status === "Online" ? "bg-emerald-500" : "bg-slate-500"} shrink-0`}
                  />
                  <p className="text-sm text-slate-700 leading-snug">
                    {device.status === "Online"
                      ? "Device connected. Streaming live."
                      : "Device disconnected."}
                  </p>
                </li>
              </ul>
              ...
              <div className="divide-y divide-slate-50">
                ...
                {[...history].reverse().map((item, index) => {
                  ...
                  if (item.heartRate! > 100) {
                    color = "bg-red-400";
                    message = "HR Spike Detected";
                  } else if (item.heartRate! > 80) {
                    color = "bg-blue-400";
                    message = "Elevated HR";
                  } else if (item.heartRate! < 60) {
                    color = "bg-teal-400";
                    message = "Low HR Detected";
                  }
                  ...
                  return (
                    ...
                    >
                      <div className="text-xs font-medium text-slate-400 w-12 pt-0.5">
                        {timeStr}
                      </div>
                      <div
                        className={`h-2 w-2 mt-1.5 rounded-full ${color} shrink-0`}
                      ></div>
                      <div>
